...
#17 pool
                if (
                    ...
                    if self.closestdronesptag:
                        ...
                        if not self.enemy_units.closer_than(3, self.start_location.position.towards(possible_base_locations[0], -10)):
                        ...
                        if self.enemy_units.closer_than(3, self.start_location.position.towards(possible_base_locations[0], -10)):
...
#lings to match enemy lings if aggression
            if (
                ...
                and self.time > 90
                and self.vespene < 25
                and self.totalvalue_e >= self.totalvalue_o
                ):
                larva.train(UnitTypeId.ZERGLING)
                #print(self.time_formatted, self.supply_used, "our combat units =", self.units.exclude_type((UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA)).amount)
                #print(self.time_formatted, self.supply_used, "enemy combat units =", self.enemy_units.exclude_type((UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA)).amount)
...
#defend against the worker rush!
        for drone in self.units(UnitTypeId.DRONE).ready:
            ...
            if (
                ...
                if (
                    self.enemy_units.not_flying.closer_than(10, self.start_location).amount > 0
                    ):
...
#lingscoutfront
            ...
            if self.closestlingtag:
                ...
                if self.closestling:
                    ...
                    if (
                        ...
                        and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(70, self.start_location).amount > 0
                        and self.enemy_units.not_flying.exclude_type(scouts).closer_than(8, self.closestling).amount < 3
                        ):
                        ...
                        not self.enemy_structures.not_flying.closer_than(70, self.start_location)
                        and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(70, self.start_location).amount > 0
                        and self.enemy_units.not_flying.exclude_type(scouts).closer_than(8, self.closestling).amount > 0
                        ):
        ...
        if not self.totalvalue_on > self.totalvalue_en and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(5, self.structures(UnitTypeId.HATCHERY).ready.closest_to(possible_base_locations[0])):
            if lings:
                print(self.time_formatted, self.supply_used, "lings =", lings)
                if self.closestling:
                    if lings.closer_than(5, self.units.exclude_type((UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA)).closest_to(possible_base_locations[0])):
...
#defend
                if (
                    self.enemy_units.not_flying.exclude_type(scouts).closer_than(50, self.start_location).amount > 0
                    and self.totalvalue_on > self.totalvalue_en
                    or self.enemy_units.not_flying.exclude_type(scouts).closer_than(5, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])).amount > 0
                    ):
                ...
                if (
                    self.enemy_structures.not_flying.closer_than(50, self.start_location)
...
#defend
            if (
                self.enemy_units.not_flying.exclude_type(scouts).closer_than(50, self.start_location).amount > 0
                and self.totalvalue_on > self.totalvalue_en
                or self.enemy_units.not_flying.exclude_type(scouts).closer_than(5, self.structures(UnitTypeId.HATCHERY).ready.closest_to(possible_base_locations[0])).amount > 0
                ):
                if (
            ...
            if (
                self.enemy_structures.not_flying.closer_than(50, self.start_location).amount > 0
                and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(50, self.start_location).amount > 0
                and self.totalvalue_on > self.totalvalue_en
                ):
                if (
...
#group
            if (
                self.totalvalue_o > 0
                and not self.enemy_structures.not_flying.closer_than(50, self.start_location).amount > 0
                and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(50, self.start_location).amount > 0
                and not self.totalvalue_o > self.totalvalue_e * 1.2
                and self.supply_used < 190
                ):
    ...
    async def build_roaches(self):
        ...
        if (
            ...
            if (
                ...
                and self.structures(UnitTypeId.ROACHWARREN).ready
                and self.units(UnitTypeId.ZERGLING).amount + self.already_pending(UnitTypeId.ZERGLING) > 0
                and not self.totalvalue_o > self.totalvalue_e
                ):
                larva.train(UnitTypeId.ROACH)
                #print(self.time_formatted, self.supply_used, "our combat units =", self.units.exclude_type((UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA)).amount)
                #print(self.time_formatted, self.supply_used, "enemy combat units =", self.enemy_units.exclude_type((UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA)).amount)
