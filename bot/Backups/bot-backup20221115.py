    ...
    async def build_workers(self):
        ...
        if (
            ...
            if (
                ...
                #print(self.time_formatted, self.supply_used, "16 hatch")
            ...
            if self.enemyworkers > 14:
                ...
                if (
                    ...
                    if (
                        ...
                        larva.train(UnitTypeId.DRONE)
                        #print(self.time_formatted, self.supply_used, "17 pool")
                if (
                    ...
                    larva.train(UnitTypeId.DRONE)
                    #print(self.time_formatted, self.supply_used, "19 hatch")
                if (
                    self.can_afford(UnitTypeId.DRONE)
                    and self.structures(UnitTypeId.HATCHERY).amount > 2
                    ...
                    if (
                        ...
                        larva.train(UnitTypeId.DRONE)
                        #print(self.time_formatted, self.supply_used, "building enough drones to match base count")
#pre move drones
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        #print(self.time_formatted, self.supply_used, "Building Gas")
                elif self.supply_workers + self.already_pending(UnitTypeId.DRONE) >= (self.structures(UnitTypeId.HATCHERY).amount * 16) + (self.structures(UnitTypeId.EXTRACTOR).amount * 3):
                    if (
                        not self.gas_buildings.closer_than(1, geysernear)
                        and not self.already_pending(UnitTypeId.EXTRACTOR)
                        and self.vespene < self.minerals
...
#lingscoutfront
        if lings:
            ...
            if self.closestlingtag:
                ...
                if self.closestling:
                    ...
                    if self.enemy_units.not_flying.exclude_type(scouts).closer_than(10, self.closestling).amount > 2:
        ...
        if self.structures(UnitTypeId.HATCHERY):
            if not self.totalvalue_on > self.totalvalue_en and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(3, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])):
                if lings:
...
#defend
                if (
                    self.enemy_units.not_flying.exclude_type(scouts).closer_than(70, self.start_location).amount > 0
                    and self.totalvalue_on > self.totalvalue_en
                    or self.enemy_units.not_flying.exclude_type(scouts).closer_than(10, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])).amount > 0
                    and self.time > 180
                    or self.enemy_units.not_flying.exclude_type(scouts).closer_than(3, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])).amount > 0
                    and self.time < 180
                    ):
                    ling.attack(self.enemy_units.not_flying.closest_to(ling))
                    print(self.time_formatted, self.supply_used, "unit defense, our nearby units =", self.totalvalue_on)
                    #print(self.time_formatted, self.supply_used, "unit defense, enemy nearby units =", self.totalvalue_en)
                if (
...
#group
                if (
                    self.totalvalue_o > 0
                    and not self.totalvalue_o > self.totalvalue_e * 1.2
                    and not self.enemy_structures.not_flying.closer_than(70, self.start_location)
                    and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(70, self.start_location).amount > 0
                    and self.supply_used < 190
                    or self.enemy_units.not_flying.exclude_type(scouts).closer_than(30, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])).amount > 0
                    and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(25, self.structures(UnitTypeId.HATCHERY).closest_to(possible_base_locations[0])).amount > 0
                    ):
...
#attack
                if (
                    ...
                    and not self.enemy_structures.not_flying.closer_than(70, self.start_location)
                    and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(70, self.start_location).amount > 0
                    and self.totalvalue_o > self.totalvalue_e * 1.2
                    and self.totalvalue_on > self.totalvalue_en
                    ):
...
#attack locations last
            if (
                not self.enemy_structures.not_flying.amount > 0
                and len(roach.orders) < 5
                and not self.enemy_units.not_flying.exclude_type((UnitTypeId.EGG, UnitTypeId.LARVA)).amount > 0
                and not self.enemy_structures.not_flying.closer_than(30, self.start_location).amount > 0
                and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(30, self.start_location).amount > 0
                and self.totalvalue_on > self.totalvalue_en
                and self.supply_used > 190
                ):
