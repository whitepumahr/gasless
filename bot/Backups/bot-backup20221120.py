...
#defend against the worker rush!
        if self.enemyworkers:
            print("enemy worker")
            naw = self.units.filter(lambda unit: unit.type_id == self.units(UnitTypeId.DRONE) and not unit.is_attacking)
            for eworker in self.enemy_units({UnitTypeId.DRONE, UnitTypeId.SCV, UnitTypeId.PROBE}):
                if eworker.position.is_closer_than(8, self.start_location):
                    print("enemy worker nearby")
                    print("our non attacking workers =", naw)
                    for drone in naw:
                        print("naw = ", naw)
                        print("eworker pos =", eworker.position)
                        print("our worker pos =", naw.position)
                        if drone.position.is_closer_than(2, eworker):
                            drone.attack(eworker).closest_to(drone)
        ...
        if lings:
            ...
            if self.closestlingtag:
                ...
                if self.closestling:
                    ...
                    if self.enemy_units.not_flying.closer_than(10, self.closestling).amount > 2 and not self.enemy_units.not_flying.exclude_type(scouts).closer_than(4, self.townhalls.closest_to(possible_base_locations[0])):
