                ...
                for enemyunit in enemies_near:
                    ...
                    if neu:
                        ...
                        for eunit in neu:
                            ...
                            self.totalvalue = 0
                    for ling in lings:
                        if not ling.position.is_closer_than(10, enemyunit):
                        ...
                        for ling in lings.closer_than(10, enemyunit):
                            ...
                            if self.townhalls:
                                if (
                                    ...
                                    ling.move(enemies_near.closest_to(ling).position.towards(ling, 11))
                                elif (
                    ...
                    if lings:
                        for ling in lings:
                            if (
                                ...
                                ling.move(allenemies.closest_to(ling).position.towards(ling, 11))
                            elif (
                                self.totalvalue_on >= self.totalvalue_en
                                ):
...
##defend
        if roaches:
            if (
                ...
                if not self.structures(UnitTypeId.SPINECRAWLER).closer_than(6, self.enemy_structures(UnitTypeId.NYDUSCANAL).closest_to(possible_base_locations[15])):
                ...
                for enemyunit in enemies_near:
                    ...
                    if neu:
                        ...
                        for eunit in neu:
                            ...
                            self.totalvalue = 0
                    for roach in roaches:
                        if not roach.position.is_closer_than(10, enemyunit):
                            roach.attack(enemies_near.closest_to(roach))
                        if roach.position.is_closer_than(10, enemyunit):
                            for roach in roach.position.is_closer_than(10, enemyunit):
                                onu = self.units.filter(lambda onu:
                                                        not onu.is_flying
                                                        and not onu.type_id in [UnitTypeId.EGG, UnitTypeId.DRONE, UnitTypeId.OVERLORD, UnitTypeId.LARVA]
                                                        and onu.position.is_closer_than(11, enemyunit))
    ...
    async def split_queens(self):
        ...
        for hatchery in self.townhalls.ready:
            if self.minerals > 100 and self.supply_used > 198:
                if self.enemy_structures(UnitTypeId.NYDUSCANAL) and not self.structures(UnitTypeId.SPINECRAWLER).closer_than(7, self.enemy_structures(UnitTypeId.NYDUSCANAL).closest_to(possible_base_locations[15])).amount > 4:
